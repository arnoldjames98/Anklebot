...
#IE lines
#.c create line [ expr { $torquerange * 3 / 4 * $pi / 180 * $scale } ] -50 [ expr { $torquerange * 3 / 4 * $pi / 180 * $scale } ] 50 -width 2 -fill yellow -tags top
#.c create line [ expr { $torquerange * 3 / 4 * $pi / 180 * $scale * -1 } ] -50 [ expr { $torquerange * 3 / 4 * $pi / 180 * $scale * -1 } ] 50 -width 2 -fill yellow -tags bottom
...
every 10 {
  ...
  #Target number is initialized value, used for probability of the distance of the targets
  ...
  if { $bz == 0} {
    if { $stableMode == 0 } {
      ...
      # Has to be switched from dp to ie for ie study
      ...
      if { $number == 1 } {
        set damp 0
        .c delete theDamp
      	.c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "No Damping" -font [list Helvetica 30] -tags theDamp
		...
		# Finding the min v times a
      	...
      	# Tuning K
        	...
        	set damp [ expr { ( $b_UB - $b_LB ) * (-1) / (1 + exp( (-1) * $avg_k *$vel * $accel) ) + $b_UB } ]
        	 .c delete theDamp
      		.c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Variable Damping Tuning: k = $avg_k" -font [list Helvetica 30] -tags theDamp
      ...
      # Variable Damping
        ...
        set damp [ expr { ( $b_UB - $b_LB) * (-1) / (1 + exp( (-1) * $avg_k * $vel * $accel) ) + $b_UB } ]
         .c delete theDamp
      	.c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Variable Damping Trials: k = $avg_k" -font [list Helvetica 30] -tags theDamp
      ...
      # Negative damping
        ...
        set damp -1.0
        .c delete theDamp
        .c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Negative Damping Trials" -font [list Helvetica 30] -tags theDamp
      ...
      #Positive damping
        ...
        set damp 2.0
        .c delete theDamp
        .c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Positive Damping Trials" -font [list Helvetica 30] -tags theDamp
  ...
  if { $t < 30 } {
    ...
    .c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -3.5 * $pi / 180 * $scale } ] -text "GO" -font [list Helvetica 100] -tags go
  ...
  if { $n > $RAND } {
    ...
    if { $q < $rest } {
      ...
      .c delete theTrial
      .c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -8 * $pi / 180 * $scale } ] -text "Trial $trialno of 191 complete" -font [list Helvetica 40] -tags theTrial
      #	puts $vecpos
      #	puts $stablevecpos
      ...
      .c create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -8 * $pi / 180 * $scale } ] -text "Trial $trialno of 191 complete" -font [list Helvetica 40] -tags theTrial
        ...
        #newlog
        puts "New Log Created. Here is the current"
    ...
    #    set stablevecpos [ expr { $stablevecpos +1 }]
      ...
      .c itemconfigure target -fill {light sea green}
