...
set cursor [.cfbemg create oval [ballxy 100 100 [ expr { 0.5 * $pi / 180 * $scale } ]] -fill red]

.cfbemg config -scrollregion [list -495 -495 495 495]
...
#.cfbemg create oval [ballxy $calx $caly [ expr { 1 * $pi / 180 * $scale } ]] -fill grey50 -tags cal
.cfbemg create oval [ballxy $calx $caly [ expr { 0.5 * $pi / 180 * $scale } ]] -fill grey50 -tags target
.cfbemg create line $calx 100 $calx [expr { 100 * -1 }] -width 5 -fill blue -tags cross
.cfbemg create line [expr { 100 * -1 }] $caly 100 $caly  -width 5 -fill blue -tags cross
#.cfbemg create line -50 [ expr { $torquerange * $pi / 180 * $scale } ] 50 [ expr { $torquerange * $pi / 180 * $scale } ] -width 2 -fill yellow -tags top
#.cfbemg create line -50 [ expr { $torquerange * $pi / 180 * $scale * -1 } ] 50 [ expr { $torquerange * $pi / 180 * $scale * -1 } ] -width 2 -fill yellow -tags bottom
...
if { $studyDirection == 0 } {
  ...
  #IE lines
  .cfbemg create line [ expr { $torquerange * $pi / 180 * $scale } ] -50 [ expr { $torquerange * $pi / 180 * $scale } ] 50 -width 2 -fill yellow -tags top
  .cfbemg create line [ expr { $torquerange * $pi / 180 * $scale * -1 } ] -50 [ expr { $torquerange * $pi / 180 * $scale * -1 } ] 50 -width 2 -fill yellow -tags bottom
}
...
every 10 {
  ...
  # Called while the ACTIVE Button is pressed
  if { $c == 0 } {
    if { $trialStatus == 1 } {
    # stableMode is now "Unstable"
      set stableMode 1
  ...
  if { $j == 1 } {
    ...
    .cfbemg itemconfigure cross -fill black
    ...
    .cfbemg itemconfigure cross -fill blue
  }
  ...
  if { $bz == 0} {
    if { $stableMode == 0 } {
      ...
      if { $number == 1 } {
        set damp 0
        .cfbemg delete theDamp
      	.cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "No Damping" -font [list Helvetica 30] -tags theDamp

      	set trialType "_noDamping"

      	# Used to determine K
      	set vtimesa [ expr {$vel * $accel} ]

      	# Logs this value too
      	wshm ankle_vel_times_accel $vtimesa
		...
		# Finding the min v times a
      	...
      	# Tuning K
        	...
        	set damp [ expr { ( $b_UB - $b_LB ) * (-1) / (1 + exp( (-1) * $avg_k *$vel * $accel) ) + $b_UB } ]
        	 .cfbemg delete theDamp
      		.cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Variable Damping Tuning: k = $avg_k" -font [list Helvetica 30] -tags theDamp
      
      		set trialType "_tuning"
      ...
      # Variable Damping
        ...
        set damp [ expr { ( $b_UB - $b_LB) * (-1) / (1 + exp( (-1) * $avg_k * $vel * $accel) ) + $b_UB } ]
         .cfbemg delete theDamp
      	.cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Variable Damping Trials: k = $avg_k" -font [list Helvetica 30] -tags theDamp

      	set trialType "_variable"

      # Negative damping
      } elseif { $number == 4 } {
        set damp $b_LB
        .cfbemg delete theDamp
        .cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Negative Damping Trials" -font [list Helvetica 30] -tags theDamp

        set trialType "_negative"

      #Positive damping
      } elseif { $number == 5 } {
        set damp $b_UB
        .cfbemg delete theDamp
        .cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -6 * $pi / 180 * $scale } ] -text "Positive Damping Trials" -font [list Helvetica 30] -tags theDamp
      
        set trialType "_positive"
      }
    ...
    # Stiffness when going back to the center target
  ...
  if { $t < 30 } {
    #        .cfbemg coords $::cursor [ballxy $calx $caly [ expr { 0.5 * $pi / 180 * $scale } ]]
    .cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -3.5 * $pi / 180 * $scale } ] -text "GO" -font [list Helvetica 100] -tags go
    ...
    if { $activationMode == 1 } {
      ...
      set x [rshm ankle_ie_pos]
      set x [expr { $x * $scale }]
      .cfbemg coords $::cursor [ballxy $x $caly [ expr { 0.5 * $pi / 180 * $scale } ]]
      .cfbemg raise $::cursor
      ...
      global y
      # move yellow cursor ball to match it.
      .cfbemg coords $::cursor [ballxy $calx $y [ expr { 0.5 * $pi / 180 * $scale } ]]
      .cfbemg raise $::cursor
      ...
      global y
      # move yellow cursor ball to match it.
      .cfbemg coords $::cursor [ballxy $x $y [ expr { 0.5 * $pi / 180 * $scale } ]]
      .cfbemg raise $::cursor
  ...
  if {$disttarg < [ expr { 0.5 * $pi / 180 * $scale } ]} {
    ...
    .cfbemg itemconfigure target -width 4.0
  } elseif {$disttarg > [ expr { 0.5 * $pi / 180 * $scale } ]} {
    set m 0
    .cfbemg itemconfigure target -width 1.0
  }
  ...
  if { $n > $RAND } {
    ...
    set bz 1
    if { $q < $rest } {
      .cfbemg itemconfigure target -fill green
      .cfbemg coords target [ballxy $calx $caly [ expr { 1 * $pi / 180 * $scale } ]]
      set targx $calx
      set targy $caly
      set damp 0
      ...
      # What changes the type of damping
      #if { $vecpos < 10 } {
       # set vecpos [expr { $vecpos + 1 }]
       # puts $vecpos
      #} elseif { $vecpos == 10 } {
      ...
      # Reset the time matrix NEWNEW
      #source ../tcl/graph.tcl
      source /home/imt/imt/robot4/protocols/ankle/Graph/tcl/graph.tcl

		if {$trialno > 1 && $trialno < 42} {
			#emu_graph::emu_graph foo -canvas .new_win -width 900 -height 300
			emu_graph::emu_graph foo -canvas .c -width 600 -height 300 -xref -300 -yref 150
      ...
      if { $studyDirection == 0 } {
		...
		# Used to update k for all of the tuning trials
		if {$trialno == 11 || $trialno == 21 || $trialno == 31 || $trialno == 41 } {
			set avg_k 1
			puts "Average k from the last 10 trials:"
			proc lavg L {expr ([join $L +])/[llength $L].}
			set avg_k [lavg $kMatrix]
			puts $avg_k
			# Add the average k to the logging file
			wshm ankle_varDamp_K $avg_k

			#Reset kMatrix for next average
			set kMatrix { }
		}

		# Reset to find min and max again
		set maxVal -9999998; 
		set minVal 9999998;
      ...
      puts "Trial $trialno of 191 complete"
      .cfbemg delete theTrial
      .cfbemg create text [ expr { 0 * $pi / 180 * $scale } ] [ expr { -8 * $pi / 180 * $scale } ] -text "Trial $trialno of 191 complete" -font [list Helvetica 40] -tags theTrial
      #	puts $vecpos
      #	puts $stablevecpos

      # Breaks every ten tirals
      if { [ expr $trialno % 10 ] == 1 && $trialno != 1 && $trialno != 191 } {
    ...
    #    set stablevecpos [ expr { $stablevecpos +1 }]
      ...
      # Change the damping enviornment (used to be tied to breaks)
      if {$trialno == 11 || $trialno == 41 || $trialno == 91 || $trialno == 141 } {
			set stablevecpos [ expr { $stablevecpos + 1 }]
		}
      ...
      if { $activationMode == 1} {
        if { [lindex $rdf $vecpos] == 1} {
          .cfbemg itemconfigure target -fill green
          .cfbemg coords target [ballxy $x1 $caly [ expr { 1 * $pi / 180 * $scale } ]]
          set targx $x1
          set targy $caly
        } elseif { [lindex $rdf $vecpos] == 2} {
          .cfbemg itemconfigure target -fill green
          .cfbemg coords target [ballxy $x2 $caly [ expr { 1 * $pi / 180 * $scale } ]]
          set targx $x2
          set targy $caly
        }
      } elseif { $activationMode == 2} {
        if { [lindex $rdf $vecpos] == 1} {
          .cfbemg itemconfigure target -fill green
          .cfbemg coords target [ballxy $calx $y1 [ expr { 1 * $pi / 180 * $scale } ]]
          set targx $calx
          set targy $y1
        } elseif { [lindex $rdf $vecpos] == 2} {
          .cfbemg itemconfigure target -fill green
          .cfbemg coords target [ballxy $calx $y2 [ expr { 1 * $pi / 180 * $scale } ]]
          set targx $calx
          set targy $y2
        }
      }
      ...
      .cfbemg itemconfigure target -fill {light sea green}
