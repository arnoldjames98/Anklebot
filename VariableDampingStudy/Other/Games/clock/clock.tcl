...
proc after_info {} {foreach i [after info] {puts "$i [after info $i]"}}
...
proc onceg1 {} {
    ...
    menu_t $m menu "Hide Menu (Alt-m)" ""
    menu_b $m quit "Quit (q)" {clock_exit}
    ...
    bind . <Alt-m> {puts "Clock menu feature has been removed."}
...
# hand state visual indicator for pick game

proc grasp_init_indicate {} {
# dtime "grasp_init_indicate"
    # this used to be a pacman shaped figure,
    # it is now a broad dashed token
}
...
proc hand_grasp_cancel {} {
    after cancel hand_grasp_open
    after cancel hand_grasp_close
    after cancel hand_grasp_open_wait
    after cancel grasp_generate_press
    after cancel grasp_generate_release
}

proc hand_grasp_close {{ticks 200}} {
    global ob
...
# is this better with point control?
    if {$ob(hand_collapse)} {
	set src [point_to_collapse $src $dest]
    }

# dtime "hand_grasp_close"
# dtime "movebox 1 22 (0 $ticks 1) ($src) ($dest)"
    movebox 1 22 [list 0 $ticks 1] $src $dest
    set ob(hand_position) closed
}

# open and hold

proc hand_grasp_open {{ticks 200}} {
    global ob
...
# check whether full grasp hit has occurred.
# If not, grasp_reach_first_approach will still be false.
# release hand force to allow
# user to try again without force.

proc hand_grasp_check_release {} {
    global ob
...
# open and cancel force

proc hand_grasp_open_and_release {{ticks 200}} {
    global ob
...
# dtime "hand_grasp_open_and_release"
    hand_grasp_open
    set ms [expr int($ticks * 5)]
    after $ms hand_grasp_release
}

# close and open for reach

proc hand_grasp_close_and_open {{ticks 200}} {
    global ob
...
# dtime "hand_grasp_close and open"
    hand_grasp_cancel
    hand_grasp_close
    set ms [expr int($ticks * 5)]
    set ms2 [expr int($ticks * 10 + 10)]
    after $ms hand_grasp_open
    after $ms2 hand_grasp_check_release
}


# open and close, was for pick, is not used there.

proc hand_grasp_open_and_close {{ticks 200}} {
    global ob
...
# dtime "hand_grasp_open and close"
    hand_grasp_cancel
    hand_grasp_open
    set ms [expr {int($ticks * 5) + 250}]
    after $ms hand_grasp_close
}

# non-collapsing non-moving wait slot

proc hand_grasp_open_wait {} {
    global ob
...
# dtime "hand_grasp_open_wait"
# dtime "movebox 1 22 (0 0 1) ($src) ($dest)"
    movebox 1 22 [list 0 0 1] $src $dest
    # sort of open
    set ob(hand_position) open
}
...
# dtime "ltb sched do_slot_timeout $ob(slottimeout) timo $timo"
	...
	if {$cause == "hit" && $ob(grasp_carrying_token)} {
	    after 500 grasp_pick_show_token $::ob(bigcan)
	    after 500 hand_grasp_close
	}
	if {$cause == "hit" && !$ob(grasp_carrying_token)} {
	    grasp_pick_show_token $::ob(bigcan)
	    after 1000 hand_grasp_close
	}
	if {$cause == "slot_timeout"} {
	    after 500 grasp_pick_show_token $::ob(bigcan)
	    after 1000 hand_grasp_close
	}
	if {$cause == "unpause"} {
...
# a slot, and the can be cancelled *during a slot.
...
# dtime "pause_target white, blinking $ob(blinking)"
    ...
    if {$ob(grasp_pick_game) && $ob(hand_position) == "closed"} {
	hand_grasp_open
    }
    if {$ob(grasp_reach_game) && $ob(hand_position) == "closed"} {
	hand_grasp_open
    }
